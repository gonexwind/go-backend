...
func GetProfile(c *gin.Context) {
	// Ambil data dari JWT dengan claim (misal "user_id" yang di-set saat generate token)
	userIDInterface, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "Unauthorized - user_id tidak ditemukan di JWT",
		})
		return
	}
	userID := userIDInterface.(uint) // atau string/int sesuai tipe aslinya

	// Inisialisasi user
	var user models.User

	// Cari user di database sesuai id dari JWT
